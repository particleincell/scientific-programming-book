...
public:	
	...
	double getCellVolume() const {return dh[0]*dh[1];} //cell volume
	...
	bool isLastTimeStep() const {return ts==num_ts;}
	...
	int2 XtoIJK(const double3 &x) const {
		double2 lc = XtoL(x);
	...
	/*stores information about the circle*/
	void addCircle(const double2 &x0, double radius);
	
	/*return true if point x is inside or on the sphere*/
	bool inCircle(const double3 &x);

	/*marks the k=0 face as Dirichlet*/
	void addInlet();

	/*returns the parametric position for the intersection point*/
	double lineCircleIntersect(const double3 &x1, const double3 &x2);

	/*returns random diffuse velocity sampled at a given surface point*/
	double3 circleDiffuseVector(const double3 &x);
