...
def make_plot(it): 
    ...
    fig1.clear()
    img=pl.contourf(pos_z,pos_r,np.transpose(w),cmap='gray_r')
    pl.colorbar(img)
    pl.axis('equal')
    ...
    img=pl.contourf(pos_z,pos_r,
                np.transpose(psi),cmap='gray_r',
                )
    #pl.colorbar(img)
    (Z,R)=np.meshgrid(pos_z,pos_r)
    pl.scatter(Z,R,c=np.transpose(node_type),cmap='gray')
    pl.axis('equal')
    pl.xlim((pos_z[0],pos_z[ni-1]))
    pl.ylim((pos_r[0],pos_r[nj-1]))
    ...
    fig3.clear()
    img=pl.contourf(pos_z,pos_r,
                np.transpose(np.sqrt(u*u+v*v)),cmap='gray_r'
#                np.transpose(u)
#                ,levels=np.linspace(0,0.30,num=10)
                )
    #pl.colorbar(img)
    #pl.quiver(pos_z,pos_r, np.transpose(u), np.transpose(v))  
    pl.streamplot(pos_z,pos_r, np.transpose(u), np.transpose(v),color='0.5')  
    pl.scatter(Z,R,c=np.transpose(node_type.clip(min=0)),cmap='gray_r',s=5)
...
pl.close('all')
#pl.ioff()
fig1=pl.figure(1,figsize=(8,4))
fig2=pl.figure(2,figsize=(8,4))
fig3=pl.figure(3,figsize=(8,4))
...
pl.figure(fig2)
pl.savefig('psi.pdf',dpi=300)
pl.figure(fig3)
pl.savefig('vel.pdf',dpi=300)
print ("Done!")
